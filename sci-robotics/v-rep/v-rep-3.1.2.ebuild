...
M_PV="${PV//./_}"
SRC_URI="x86? ( ${HOMEPAGE}/V-REP_PRO_EDU_V${M_PV}_Linux.tar.gz )
amd64? ( ${HOMEPAGE}/V-REP_PRO_EDU_V${M_PV}_Linux.tar.gz )
${HOMEPAGE}/V-REP_SOURCE_V${M_PV}.zip
dynamic? ( ${HOMEPAGE}/V-REP_DYN_PLUGIN_SOURCE_V${M_PV}.zip )
mesh? ( ${HOMEPAGE}/V-REP_MESH_PLUGIN_SOURCE_V${M_PV}.zip )
path? ( ${HOMEPAGE}/V-REP_PATH_PLUGIN_SOURCE_V${M_PV}.zip )
"
...
src_unpack() {
	unpack ${A}
	cd ${WORKDIR}
	mv V-REP_PRO_EDU_V${M_PV}_Linux ${P}
	mv v_rep ${P}
...
src_prepare() {
	rm $(find . -name "*.so*")
	epatch ${FILESDIR}/${PN}-3.0.3-lua.patch
	epatch ${FILESDIR}/${PN}-3.0.3-IDSN.patch
	epatch ${FILESDIR}/${PN}-3.0.3-lib.pro.patch
	epatch ${FILESDIR}/${PN}-3.0.3-lib.patch
	epatch ${FILESDIR}/${PN}-3.0.3-urdf.patch
	use dynamic && epatch ${FILESDIR}/${PN}-3.0.3-dynamics.patch
	epatch ${FILESDIR}/${PN}-3.0.3-path.patch
